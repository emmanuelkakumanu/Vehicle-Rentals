...
let BookingComponent = class BookingComponent {
    constructor(vehicleService, route, fb, bookingService, router, userAuthService, userService) {
    ...
    ngOnInit() {
        ...
        this.route.params.subscribe((params) => {
            ...
            this.userService.getUser(this.userAuthService.getId()).subscribe((user) => {
                console.log(user);
...
let MyBookingComponent = class MyBookingComponent {
    constructor(bookingService, userAuthService) {
        this.bookingService = bookingService;
        this.userAuthService = userAuthService;
    }
    ngOnInit() {
...
MyBookingComponent.ctorParameters = () => [
...
let PaymentComponent = class PaymentComponent {
    constructor(fb, bookingService, router, route, userAuthService) {
        this.fb = fb;
        this.bookingService = bookingService;
        this.router = router;
        this.route = route;
...
let BookingService = class BookingService {
    ...
    updatePayment(bookingDetail, payment) {
...
let LoginComponent = class LoginComponent {
    constructor(userAuthService, authenticationService, router, fb, route) {
        this.userAuthService = userAuthService;
...
let SignupComponent = class SignupComponent {
    constructor(fb, router, userService, userAuthService) {
        this.fb = fb;
        this.router = router;
        this.userService = userService;
        this.userAuthService = userAuthService;
...
let ViewRequestComponent = class ViewRequestComponent {
    constructor(userAuthService, httpClient, userService) {
        this.userAuthService = userAuthService;
        this.httpClient = httpClient;
        this.userService = userService;
    }
    ngOnInit() {
        this.userService.getAllPendingUsers().subscribe((user) => {
            if (user) {
                console.log(user);
                this.user = user;
                this.error = '';
...
ViewRequestComponent.ctorParameters = () => [
    { type: src_app_service_user_auth_service__WEBPACK_IMPORTED_MODULE_3__["UserAuthService"] },
...
let SearchComponent = class SearchComponent {
    constructor(vehicleService, userAuthService, router) {
        this.vehicleService = vehicleService;
        this.userAuthService = userAuthService;
        this.router = router;
    }
...
let VehicleEditComponent = class VehicleEditComponent {
    constructor(vehicleService, route, fb) {
        this.vehicleService = vehicleService;
        this.route = route;
        this.fb = fb;
    }
...
VehicleEditComponent.ctorParameters = () => [
    { type: src_app_service_vehicle_service__WEBPACK_IMPORTED_MODULE_2__["VehicleService"] },
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_4__["ActivatedRoute"] },
...
let VehicleInfoComponent = class VehicleInfoComponent {
    constructor(userAuthService, vehicleSrvice, router) {
        this.userAuthService = userAuthService;
    ...
    onRemovingVehicle(vehicleId) {
        console.log(vehicleId);
        this.removeVehicle.emit(vehicleId);
        this.removed = true;
    }
    viewMoreDetails(vehicleId) {
        this.id = vehicleId;
        this.viewMore.emit(vehicleId);
    }
    viewLessDetails(vehicleId) {
        vehicleId = null;
        this.id = vehicleId;
...
VehicleInfoComponent.ctorParameters = () => [
...
let VehicleListComponent = class VehicleListComponent {
    ...
    ngOnInit() {
        this.vehicleService.filter.subscribe((obj) => {
            if (obj.name !== '') {
                const result = this.temporaryList.filter(vehicle => vehicle.name.toLowerCase().includes(obj.name.toLowerCase()));
